			...
			IRCMessage message)
{
	...
	server->reply(client->getConnection(), client->getNickname() + " Joined " + \
	...
	for (std::vector<Client *>::iterator it = clients.begin(); \
			it != clients.end(); ++it)
	{
		server->reply(client->getConnection(), RPL_NAMREPLY(channel->getName(), \
			(*it)->getNickname()));
	}
	server->reply(client->getConnection(), RPL_ENDOFNAMES(channel->getName()));

	if (channel->getTopic() != "")
		server->reply(client->getConnection(), "Topic of channel: " + channel->getTopic());
